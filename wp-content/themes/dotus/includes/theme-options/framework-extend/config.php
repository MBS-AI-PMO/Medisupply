...
if (function_exists('cs_framework_init')) {
  ...
  require_once( DOTUS_CS_FRAMEWORK . '/theme-options.php' );
  require_once( DOTUS_CS_FRAMEWORK . '/theme-customizer.php' );
  require_once( DOTUS_CS_FRAMEWORK . '/theme-metabox.php' );
   ...
   */
  if( ! function_exists( 'dotus_custom_font_upload' ) ) {
    function dotus_custom_font_upload( $db_value ) {
    ...
    add_action( 'cs_typography_family', 'dotus_custom_font_upload' );
  }
 ...
 * Check Custom Font
 */
if ( ! function_exists( 'dotus_custom_upload_font' ) ) {
  function dotus_custom_upload_font( $font ) {
 ...
 */
if ( ! function_exists( 'dotus_registered_sidebars' ) ) {
  function dotus_registered_sidebars() {
 ...
 */
if ( ! function_exists( 'dotus_typography_fonts' ) ) {
  function dotus_typography_fonts() {
    ...
    if ( ! empty( $embed_fonts ) ) {
      ...
      wp_enqueue_style( 'dotus-typography-fonts', esc_url( add_query_arg( 'family', urlencode( implode( '|', $query_args ) ) . $subsets, '//fonts.googleapis.com/css2' ) ), array(), null );
    }
...
/* Typography */
if ( ! function_exists( 'dotus_get_typography' ) ) {
  function dotus_get_typography() {
    ...
    if ( ! empty( $typography ) ) {
      foreach ( $typography as $font ) {
        if ( ! empty( $font['selector'] ) ) {
          ...
          $weight  = ( $font['font']['variant'] != 'regular' ) ? dotus_esc_string( $font['font']['variant'] ) : 400;
          $style   = dotus_esc_number( $font['font']['variant'] );
          ...
          $output .= ( ! empty( $font['size'] ) ) ? 'font-size: '. dotus_check_px( $font['size'] ) .';' : '';
