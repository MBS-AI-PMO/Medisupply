...
if ( ! function_exists( 'dotus_dynamic_styles' ) ) {
  function dotus_dynamic_styles() {

    // Typography
    $dotus_get_typography  = dotus_get_typography();
    ...
    // Logo
    $dotus_logo_top     = cs_get_option( 'dotus_logo_top' );
    $dotus_logo_bottom  = cs_get_option( 'dotus_logo_bottom' );
...
$layout_boxed = ( $bg_pattern === 'custom-pattern' ) ? 'background-image: url('. esc_url($custom_bg_pattern) .');' : 'background-image: url('. esc_url(DOTUS_IMAGES) . '/patterns/'. $bg_pattern .'.png);';
?>
  .layout-boxed {
    <?php echo   wp_kses_data( $layout_boxed ); ?>
  }
...
if ($dotus_meta) {
  $transparent_header =  isset( $dotus_meta['transparency_header'] ) ? $dotus_meta['transparency_header'] : '' ;
  $transparent_menu_color = isset( $dotus_meta['transparent_menu_color'] ) ? $dotus_meta['transparent_menu_color'] : '' ;
  $transparent_menu_hover = isset( $dotus_meta['transparent_menu_hover_color'] ) ? $dotus_meta['transparent_menu_hover_color'] : '' ;
} else {
  $transparent_header = '';
...
<?php
  ...
  echo   wp_kses_data( $dotus_get_typography );
  $output = ob_get_clean();
  return $output;
 ...
 * Custom Font Family
 */
if ( ! function_exists( 'dotus_custom_font_load' ) ) {
  function dotus_custom_font_load() {
...
/* Custom Styles */
if( ! function_exists( 'dotus_custom_css' ) ) {
  function dotus_custom_css() {
    wp_enqueue_style('dotus-default-style', get_template_directory_uri() . '/style.css');
    $output  = dotus_custom_font_load();
    $output .= dotus_dynamic_styles();

    if( function_exists( 'dotus_compress_css_lines' ) ) {
      $custom_css = dotus_compress_css_lines( $output );
       wp_add_inline_style( 'dotus-default-style', $custom_css );
    }
    dotus_typography_fonts();
  }
  add_action( 'wp_enqueue_scripts', 'dotus_custom_css' );
}

/* Custom JS */
if( ! function_exists( 'dotus_custom_js' ) ) {
  function dotus_custom_js() {
    $output = cs_get_option( 'theme_custom_js' );
    wp_add_inline_script( 'jquery-migrate', $output );
  }
  add_action( 'wp_enqueue_scripts', 'dotus_custom_js' );
}