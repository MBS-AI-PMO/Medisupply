...
class Breadcrumb_Trail {
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:', 'dotus' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'dotus' ),
			'home'                => esc_html__( 'Home', 'dotus' ),
			'error_404'           => esc_html__( '404 Not Found', 'dotus' ),
			'archives'            => esc_html__( 'Archives', 'dotus' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;', 'dotus' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', 'dotus' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'dotus' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'dotus' ),
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'dotus' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'dotus' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'dotus' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'dotus' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'dotus' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format', 'dotus' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'dotus' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'dotus' ) ) );
