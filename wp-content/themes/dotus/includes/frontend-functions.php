...
if( ! function_exists( 'dotus_excludecat' ) ) {
  function dotus_excludecat($query) {
  	if ( $query->is_home ) {
  ...
  add_filter('pre_get_posts', 'dotus_excludecat');
}
...
class DotusExcerpt {
  ...
  public static function length($new_length = 55) {
    DotusExcerpt::$length = $new_length;
    add_filter('excerpt_length', 'DotusExcerpt::new_length');
    DotusExcerpt::output();
  }

  // Tells WP the new length
  public static function new_length() {
    if( isset(DotusExcerpt::$types[DotusExcerpt::$length]) )
      return DotusExcerpt::$types[DotusExcerpt::$length];
    else
      return DotusExcerpt::$length;
  }
...
if( ! function_exists( 'dotus_excerpt' ) ) {
  function dotus_excerpt($length = 55) {
    DotusExcerpt::length($length);
  }
}

if ( ! function_exists( 'dotus_new_excerpt_more' ) ) {
  function dotus_new_excerpt_more( $more ) {
    return ' ';
  }
  add_filter('excerpt_more', 'dotus_new_excerpt_more');
}

/* Tag Cloud Widget - Remove Inline Font Size */
if( ! function_exists( 'dotus_tag_cloud' ) ) {
  function dotus_tag_cloud($tag_string){
    return preg_replace("/style='font-size:.+pt;'/", '', $tag_string);
  }
  add_filter('wp_generate_tag_cloud', 'dotus_tag_cloud', 10, 3);
}

/* Password Form */
if( ! function_exists( 'dotus_password_form' ) ) {
  function dotus_password_form( $output ) {
    $output = str_replace( 'type="submit"', 'type="submit" class=""', $output );
    return $output;
  }
  add_filter('the_password_form' , 'dotus_password_form');
}


/* Widget Layouts */
if ( ! function_exists( 'dotus_footer_widgets' ) ) {
  function dotus_footer_widgets() {
...
/* WP Link Pages */
if ( ! function_exists( 'dotus_wp_link_pages' ) ) {
  function dotus_wp_link_pages() {
    $defaults = array(
      'before'           => '<div class="wp-link-pages">' . esc_html__( 'Pages:', 'dotus' ),
      'after'            => '</div>',
      'link_before'      => '<span>',
      'link_after'       => '</span>',
...
/* Metas */
if ( ! function_exists( 'dotus_post_metas' ) ) {
  function dotus_post_metas() {
  ...
  <div class="bp-top-meta">
    ...
    <div class="bp-author">
      <?php
      printf(
        '<span>'. esc_html__('by','dotus') .' <a href="%1$s" rel="author">%2$s</a></span>',
...
/* Author Info */
if ( ! function_exists( 'dotus_author_info' ) ) {
  function dotus_author_info() {
    ...
    $author_text = $author_text ? $author_text : esc_html__( 'Author', 'dotus' );
...
=============================================== */
if ( ! function_exists( 'dotus_comment_modification' ) ) {
  function dotus_comment_modification($comment, $args, $depth) {
  ...
  <<?php echo esc_attr($tag); ?> <?php comment_class('item ' . $comment_class .' ' ); ?> id="comment-<?php comment_ID() ?>">
   ...
   <article>
    <div id="div-comment-<?php comment_ID() ?>" class="dotus-comment">
    <?php endif; ?>
    ...
    <div class="comment-main-area">
      ...
      <em class="comment-awaiting-moderation"><?php echo esc_html__( 'Your comment is awaiting moderation.', 'dotus' ); ?></em>
      <?php endif; ?>
      <div class="comment-content">
        ...
        <div class="comments-reply">
          <?php
            ...
            'reply_text' => '<span class="comment-reply-link icofont icofont-reply-all">'.esc_html__( 'Reply', 'dotus' ).'</span>',
            'before' => '',
            'class'  => '',
            'depth' => $depth,
...
if( ! function_exists( 'dotus_move_comment_field' ) ) {
  add_filter( 'comment_form_fields', 'dotus_move_comment_field' );
  function dotus_move_comment_field( $fields ) {
...
/* Title Area */
if ( ! function_exists( 'dotus_title_area' ) ) {
  function dotus_title_area() {
    ...
    if ($dotus_meta && (!is_archive() || is_woocommerce_shop())) {
      $custom_title = $dotus_meta['page_custom_title'];
      if ($custom_title) {
    ...
    if ( is_home() && is_front_page() ) {
      echo esc_html__( 'Blog', 'dotus' );
    } elseif ( is_home() && !is_front_page() ) {
      single_post_title();
    } elseif ( is_search() ) {
      printf( esc_html__( 'Search Results for %s', 'dotus' ), '<span>' . get_search_query() . '</span>' );
    } elseif ( is_category() || is_tax() ){
      single_cat_title();
    } elseif ( is_tag() ){
      single_tag_title( esc_html__('Posts Tagged: ', 'dotus') );
    } elseif ( is_archive() ){
      if ( is_day() ) {
        printf( wp_kses( __( 'Archive for <span>%s</span>', 'dotus' ), array( 'span' => array() ) ), get_the_date());
      } elseif ( is_month() ) {
        printf( wp_kses( __( 'Archive for <span>%s</span>', 'dotus' ), array( 'span' => array() )), get_the_date( 'F, Y' ));
      } elseif ( is_year() ) {
        printf( wp_kses( __( 'Archive for <span>%s</span>', 'dotus' ), array( 'span' => array() ) ), get_the_date( 'Y' ));
      } elseif ( is_author() ) {
        printf( wp_kses( __( 'Posts by: <span>%s</span>', 'dotus' ), array( 'span' => array() ) ), get_the_author_meta( 'display_name', $wp_query->post->post_author ));
      } elseif( is_woocommerce_shop() ) {
        esc_html_e( 'Shop', 'dotus' );
      } elseif( is_product() ) {
        esc_html_e( 'Shop Single', 'dotus' );
      } elseif ( is_post_type_archive() ) {
        post_type_archive_title();
      } else {
        esc_html_e( 'Archives', 'dotus' );
      }
      ...
      esc_html_e('Team Single', 'dotus');
    }  elseif( is_404() ) {
      esc_html_e('404 Error', 'dotus');
    } elseif( $custom_title ) {
...
// Pingback Head
function dotus_pingback_header() {
  if ( is_singular() && pings_open() ) {
    printf( '<link rel="pingback" href="%s">' . "\n", get_bloginfo( 'pingback_url' ) );
  }
}
add_action( 'wp_head', 'dotus_pingback_header' );


/**
 * blog pagination function.
 */
if ( ! function_exists( 'dotus_posts_navigation' ) ) :
  function dotus_posts_navigation() {
 ...
 */
if ( ! function_exists( 'dotus_single_pagination' ) ) :
  function dotus_single_pagination() {
    $newer_post = cs_get_option('newer_post');
    $newer_post = ( $newer_post ) ? $newer_post : esc_html__('Next Post', 'dotus');
    $older_post = cs_get_option('older_post');
    $older_post = ( $older_post ) ? $older_post : esc_html__('Previous Post', 'dotus');
    the_post_navigation(
      array(
        'next_text' => '<span class="meta-nav" aria-hidden="true">' . esc_html( 'Next Post', 'dotus' ) . '</span> ' .
          '<span class="screen-reader-text">' . esc_html( $newer_post ) . '</span> <br/>' .
          '<span class="post-title">%title</span>',
        'prev_text' => '<span class="meta-nav" aria-hidden="true">' . esc_html( 'Previous Post', 'dotus' ) . '</span> ' .
...
function dotus_unique_id( $prefix = '' ) {
  static $id_counter = 0;
...
if (!function_exists('dotus_header_function')) {
  function dotus_header_function(){
  ...
  $dotus_meta  = get_post_meta( $dotus_id, 'page_type_metabox', true );
  // Theme Layout Width
  $dotus_layout_width  = cs_get_option( 'theme_layout_width' );
  $theme_preloder  = cs_get_option( 'theme_preloder' );
  $dotus_layout_width_class = ( $dotus_layout_width === 'container' ) ? 'layout-boxed' : 'layout-full';
  // Header Style
  if ( $dotus_meta ) {
    $dotus_header_design  = $dotus_meta['select_header_design'];
    if ($dotus_header_design != 'theme') {
      $dotus_header_design = $dotus_header_design;
    } else {
  ...
  if (is_numeric($dotus_header_design)) {
  ...
  <body <?php body_class(); ?>>
     <?php wp_body_open(); ?>
  <div class="page-wrapper <?php echo esc_attr( $dotus_layout_width_class ); ?>"> <!-- #dotus-theme-wrapper -->
  <?php if( $theme_preloder ) {
   get_template_part( 'theme-layouts/header/preloder' );
    ...
    <header id="header" class="<?php echo esc_attr( $header_class ); ?>">
      <?php  if (is_numeric($dotus_header_design)) {
        $header_builder = new WP_Query(
          array(
            'post_type' => 'headerbuilder',
            'posts_per_page' => 1,
            'p' => $dotus_header_design,
            'orderby' => 'none',
            'order' => 'DESC'
          )
        );
    ...
    <?php
    // Title Area
    $dotus_need_title_bar = cs_get_option('need_title_bar');
    if ( !$dotus_need_title_bar ) {
