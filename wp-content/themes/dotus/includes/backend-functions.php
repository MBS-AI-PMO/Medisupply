<?php
/*
 * All Back-End Helper Functions for Dotus Theme
 * Author & Copyright:wpoceans
 * URL: http://themeforest.net/user/wpoceans
 */

/* Validate px entered in field */
if( ! function_exists( 'dotus_check_px' ) ) {
  function dotus_check_px( $num ) {
    return ( is_numeric( $num ) ) ? $num . 'px' : $num;
  }
}

/* Escape Strings */
if( ! function_exists( 'dotus_esc_string' ) ) {
  function dotus_esc_string( $num ) {
    return preg_replace('/\D/', '', $num);
  }
}

/* Escape Numbers */
if( ! function_exists( 'dotus_esc_number' ) ) {
  function dotus_esc_number( $num ) {
...
if ( ! function_exists( 'dotus_tinymce_btns_font' ) ) {
  function dotus_tinymce_btns_font( $buttons ) {
    array_unshift( $buttons, 'fontselect' ); // Add Font Select
    array_unshift( $buttons, 'fontsizeselect' ); // Add Font Size Select
    return $buttons;
  }
  add_filter( 'mce_buttons_2', 'dotus_tinymce_btns_font' );
}

// Customize mce editor font sizes
if ( ! function_exists( 'dotus_tinymce_sizes' ) ) {
  function dotus_tinymce_sizes( $initArray ){
    $initArray['fontsize_formats'] = "9px 10px 12px 13px 14px 16px 18px 21px 24px 28px 32px 36px";
    return $initArray;
  }
  add_filter( 'tiny_mce_before_init', 'dotus_tinymce_sizes' );
}

// Customize mce editor font family
if ( ! function_exists( 'dotus_tinmymce_family' ) ) {
  function dotus_tinmymce_family( $initArray ) {
  ...
  add_filter( 'tiny_mce_before_init', 'dotus_tinmymce_family' );
}

/* HEX to RGB */
if( ! function_exists( 'dotus_hex2rgb' ) ) {
  function dotus_hex2rgb( $hexcolor, $opacity = 1 ) {
...
if ( is_admin() ) {
  ...
  if( ! function_exists( 'dotus_remove_rev_slider_meta_boxes' ) ) {
    function dotus_remove_rev_slider_meta_boxes() {
      remove_meta_box( 'dotus_metabox_revslider_0', 'page', 'normal' );
      remove_meta_box( 'dotus_metabox_revslider_0', 'post', 'normal' );
      remove_meta_box( 'dotus_metabox_revslider_0', 'service', 'normal' );
      remove_meta_box( 'dotus_metabox_revslider_0', 'testimonial', 'normal' );
      remove_meta_box( 'dotus_metabox_revslider_0', 'portfolio', 'normal' );
    }
    add_action( 'do_meta_boxes', 'dotus_remove_rev_slider_meta_boxes' );
  }

}

function dotus_project_add_icons_to_codestar_iconset() {

    $title  = esc_html__( 'Flat Icons', 'dotus' );
    $icons  = array(
        'flaticon-credit',
        'flaticon-job',
        'flaticon-team-work',
...
add_action( 'cs_add_icons', 'dotus_project_add_icons_to_codestar_iconset' );


function dotus_kingcomposer_fonts() {
...
add_action('init', 'dotus_kingcomposer_fonts');
