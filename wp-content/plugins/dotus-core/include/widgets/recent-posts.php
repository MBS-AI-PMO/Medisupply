 ...
 */
class dotus_recent_posts extends WP_Widget {
  ...
  public function __construct() {
    parent::__construct(
      '-recent-blog',
      DOTUS_THEME_NAME_PLUGIN . esc_html__( ': Recent Posts', 'dotus' ),
      array(
        'classname'   => 'recent-post-widget',
        'description' => DOTUS_THEME_NAME_PLUGIN . esc_html__( ' widget that displays recent posts.', 'dotus' )
      )
    );
  }
  ...
  public function form( $instance ) {
    ...
    $instance   = wp_parse_args( $instance, array(
      'title'    => esc_html__( 'Recent Posts', 'dotus' ),
      'ptypes'   => 'post',
      'limit'    => '3',
      'date'     => true,
      'category' => '',
    ...
    $ptypes_field = array(
      ...
      'type' => 'select',
      'options' => 'post_types',
      'default_option' => esc_html__( 'Select Post Type', 'dotus' ),
      'title' => esc_html__( 'Post Type :', 'dotus' ),
    );
    ...
    $limit_field = array(
      ...
      'type'  => 'text',
      'title' => esc_html__( 'Limit :', 'dotus' ),
      'help' => esc_html__( 'How many posts want to show?', 'dotus' ),
    );
    ...
    $date_field = array(
      ...
      'title' => esc_html__( 'Display Date :', 'dotus' ),
    );
    ...
    $category_field = array(
      ...
      'type'  => 'text',
      'title' => esc_html__( 'Category :', 'dotus' ),
      'help' => esc_html__( 'Enter category slugs with comma(,) for multiple items', 'dotus' ),
    );
    ...
    $order_field = array(
      ...
      'default_option' => esc_html__( 'Select Order', 'dotus' ),
      'title' => esc_html__( 'Order :', 'dotus' ),
    );
    ...
    $orderby_field = array(
      ...
      'type' => 'select',
      'options'   => array(
        'none' => esc_html__('None', 'dotus'),
        'ID' => esc_html__('ID', 'dotus'),
        'author' => esc_html__('Author', 'dotus'),
        'title' => esc_html__('Title', 'dotus'),
        'name' => esc_html__('Name', 'dotus'),
        'type' => esc_html__('Type', 'dotus'),
        'date' => esc_html__('Date', 'dotus'),
        'modified' => esc_html__('Modified', 'dotus'),
        'rand' => esc_html__('Random', 'dotus'),
      ),
      'default_option' => esc_html__( 'Select OrderBy', 'dotus' ),
      'title' => esc_html__( 'OrderBy :', 'dotus' ),
    );
  ...
  public function widget( $args, $instance ) {
    ...
    if ($dotus_rpw->have_posts()) : while ($dotus_rpw->have_posts()) : $dotus_rpw->the_post();
      ...
      if (!empty( $post_img )) {
        ...
        $post_img = DOTUS_PLUGIN_IMGS.'/80X80.jpg';
      } 
      ?>
    <div class="post">
...
add_action( 'widgets_init', function() { register_widget( "dotus_recent_posts" ); } );