...
function dotus_insert_elementor($atts){
...
add_shortcode('dotus_elementor_template','Elementor\dotus_insert_elementor');
		...
		*/
		public static function get_posts( $post_type = '', $numberposts = ''){
			...
			if ( is_array( $gen_posts ) && !empty( $gen_posts ) ) {
			  ...
			  $all_posts = esc_html__( 'No contact forms found', 'dotus-core' );
			}
			return $all_posts;
		}
	...
	/**
		...
		*/
		public static function get_include_icons(){
			...
			// Line icon
			$line_icons = DOTUS_EM_SHORTCODE_BASE_PATH . '/lib/icons/linea.json';
			$line_icons = file_get_contents($line_icons);
			$line_icons = json_decode($line_icons)->icons;
			
			//Themify icon
			$themify_icons = DOTUS_EM_SHORTCODE_BASE_PATH . '/lib/icons/themify.json';
			$themify_icons = file_get_contents($themify_icons);
			$themify_icons = json_decode($themify_icons)->icons;

			//Themify icon
			$flat_icons = DOTUS_EM_SHORTCODE_BASE_PATH . '/lib/icons/flat.json';
