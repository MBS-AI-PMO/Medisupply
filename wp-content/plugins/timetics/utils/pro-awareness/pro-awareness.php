		...
		public function generate_grids() {
			...
			<div class="tw-help-section tw-setup-widgard-area">
				<div class="tw-help-hero">
					...
					<p><?php echo esc_html__( 'To know the timetics starting guide, run the setup wizard. Your existing settings will not change.', 'timetics' );?></p>
                                ...
                                <a target="_blank" href="<?php echo esc_url( $grid['url'] ); ?>"
                                   class="wpmet_pro_a_wrapper tw-grid-item-inner" title="<?php echo esc_attr( $grid['title'] ); ?>">
                                    <div class="wpmet_pro_a_thumb">
                                        <img src="<?php echo esc_url( $grid['thumbnail'] ); ?>" alt="<?php echo esc_attr( $grid['title'] ); ?>">
                                    </div>
                                    <!-- // thumbnail -->

                                    <h4 class="wpmet_pro_a_grid_title tw-grid-title"><?php echo esc_html( $grid['title'] ); ?></h4>
							...
							<div class="tw-accordion-wrapper">
									...
									<div class="tw-accordion-content-wrapper">
										...
										<div class="tw-accordion-content">
											...
											<div class="tw-accordion-cotent">
												<p><?php echo esc_html__('You have to give valid connection details in timetics=>Settings=>User data.', 'timetics'); ?></p>
												<p><?php echo esc_html__('After fill up “Api key”, “Secret key” enter “Save changes” and then check connection. After successful connection navigate to Timetics=>Zoom to create a meeting.', 'timetics'); ?></p>
								...
								<div class="tw-accordion-item">
									<div class="tw-accordion-content-wrapper">
										...
										<div class="tw-accordion-content">
											<h3 class="tw-accordion-title">
												<?php echo esc_html__('How to upgrade to Timetics PRO?', 'timetics'); ?>
											</h3>
											<div class="tw-accordion-cotent">
												<p><?php echo  Helper::kses('If you buy the plugin from CodeCanyon then you do not need license key activation. Just use the Envato Marketplace plugin to get regular updates. If you buy the plugin from <a href="https://themewinter.com/timetics/" target="_blank">Themewinter website</a> then follow our <a href="https://support.themewinter.com/docs/plugins/plugin-docs/general-settings-timetics/license/" target="_blank">documentation here</a> for activating the PRO license.', 'timetics'); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></p>
											</div>
										</div>
									</div>
								</div>
							...
							<div class="tw-accordion-wrapper">
								...
								<div class="tw-accordion-item">
									<div class="tw-accordion-content-wrapper">
										...
										<div class="tw-accordion-content">
											<h3 class="tw-accordion-title">
												<?php echo esc_html__('Does Timetics support different page builders?', 'timetics'); ?>
											</h3>
											<div class="tw-accordion-cotent">
												<p><?php echo esc_html__('Yes. Timetics supports different page builders like DIVI builder, Visual Composer and Elementor page builder. You can use a shortcode for DIVI builder and Visual Composer. And the plugin has 15+ dedicated widgets for the Elementor page builder. Most of the widgets are FREE to use. However, <a href="https://themewinter.com/timetics/" target="_blank">Timetics PRO</a> will be required for a few specific widgets.', 'timetics'); ?></p>
											</div>
										</div>
									</div>
								</div>
				...
				if(!empty($action_link['link']) && !empty($action_link['text'])) {
					...
					if(!empty($action_link['attr'])) {
						...
						foreach($action_link['attr'] as $key => $val) {
							...
							$attributes .= sanitize_key( $key ) . '="' . esc_attr( $val ) . '" ';
					    }
					}
			...
			if($file == $this->plugin_file) {
				...
				foreach($this->row_meta_links as $meta) {
					...
					if(!empty($meta['link']) && !empty($meta['text'])) {
						...
						$links[] = sprintf('<a href="%s" %s > %s </a>', esc_url($meta['link']), $attributes, esc_html($meta['text']));
					}
				}
