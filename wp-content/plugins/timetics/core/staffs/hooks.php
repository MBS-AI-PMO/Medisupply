...
class Hooks {
    ...
    public function make_staff() {
        ...
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Nonce verified after action check
        $user_id = isset( $_GET['users'] ) ? absint( wp_unslash( $_GET['users'] ) ) : 0;
    ...
    public function reset_email_title( $title ) {
        ...
        $title = sprintf( esc_html__( 'Welcome to %s onboarding!', 'timetics' ), esc_html( get_bloginfo( 'name' ) ) );
