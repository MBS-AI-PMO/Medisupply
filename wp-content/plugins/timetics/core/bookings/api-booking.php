...
class Api_Booking extends Api {
    ...
    protected function booking_appointment ($data, $id) {
        ...
        if ( 'created' === $action && ! $this->is_available_slot( $meeting, [
            ...
            return new WP_Error( 'time_slot_error', sprintf( __( '%s time slot is not available', 'timetics' ), esc_html( $start_time ) ) );
        }
        ...
        if ( $id ) {
            if ( 'cancel' === $status ) {
                ...
                $old_booking = new Booking( $id );
                $date_time_changed = (
                    $old_booking->get_start_date() !== $start_date ||
                    $old_booking->get_start_time() !== $start_time ||
                    $old_booking->get_end_time() !== $end_time
                );
